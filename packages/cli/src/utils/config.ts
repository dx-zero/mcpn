...
export async function loadMcpnConfig(cwd: string = process.cwd()): Promise<ResolvedMcpnConfig> {
  ...
  logger.debug(`Loaded config from: ${resolvedConfig.configFile || 'unknown source'}`);
...
export async function loadMcpnConfigOrFail(cwd?: string): Promise<ResolvedMcpnConfig> {
    ...
    if (!configFileExists(config)) {
        ...
        throw new Error(`MCPN configuration file (e.g., mcpn.config.ts) not found in ${config.cwd}. Please run 'mcpn init' or create one manually.`);
    }
    return config;
} 
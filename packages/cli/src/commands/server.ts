...
async function getPackageVersion(): Promise<string> {
  ...
  try {
    const pkgContent = await fsp.readFile(packageJsonPath, 'utf-8')
...
export default defineCommand({
  ...
  async run(ctx) {
    ...
    if (ctx.args.preset) {
      presets = ctx.args.preset.split(',').map(p => p.trim()).filter(Boolean)
    }

    if (presets.length === 0 && !configPath) {
      logger.info('No preset or config specified, defaulting to \'thinking\' preset.')
      presets = ['thinking']
    }
    ...
    try {
      ...
      logger.info(`Loading configuration with presets: ${presets.join(', ')}${configPath ? ` and config: ${configPath}`: ''}`)
